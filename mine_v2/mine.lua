...
function grab_supplies()
    ...
    r.placeDown()
    get_item_other(s.bottom, FUEL_MATERIAL, 24)
    get_item_other(s.bottom, SCAFFOLD_MATERIAL, 320)
    --get_item_other(s.bottom, "minecraft:diamond_pickaxe",1)
    get_item_other(s.bottom, "minecraft:redstone_block",4)
    get_item_other(s.bottom, "minecraft:golden_rail", 4)
    get_item_other(s.bottom, "minecraft:rail", 4)
...
function return_home()
    ...
    local init = io.open("config.txt","r")
    ...
    local hi = 0
    if init~=nil then
        print("found a config file, setting params")
        init:close()
        local lines = {}
